...
void Application::InitTWBar()
{
	//RenderDeviceDX11* pDevice = dynamic_cast<RenderDeviceDX11*>(this->m_pRenderDevice);

	//if (pDevice)
	//{
	//	//const auto& SCDesc = m_pSwapChain.Get()->GetDesc();
	//	//TwInit(TW_DIRECT3D11, m_pDevice.Get(), m_pDeviceContext.Get(), SCDesc.ColorBufferFormat, SCDesc.DepthBufferFormat);

	//	//// TODO: to get color&depth format
	//	//int ColorBufferFormat = 29;
	//	//int DepthBufferFormat = 40;

	//	//TwInit(TW_DIRECT3D11, pDevice->GetDevice().Get(), pDevice->GetDeviceContext().Get(), ColorBufferFormat, DepthBufferFormat);
	//	TwInit(TW_DIRECT3D11, pDevice->GetDevice().Get());
	//}
}
